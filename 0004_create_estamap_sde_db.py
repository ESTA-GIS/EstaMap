'''
Create Estamap database and sde connection file.
...
Options:
  ...
  --log_file <file>         Log File name. [default: 004_create_estamap_sde_db.log]
...
def create_EMsde_db(estamap_version, auth_file):
    ...
    logging.info('creating SDE geodatabase: ' + em.database_name)
    arcpy.CreateEnterpriseGeodatabase_management(database_platform="SQL_Server",
                                                 instance_name="TDB03",
                                                 database_name=em.database_name,
    ...
    logical_name_db = conn.execute('''select name from sys.master_files where database_id = db_id('{db}') and type_desc = 'ROWS' '''.format(db=em.database_name))
    logical_name_log = conn.execute('''select name from sys.master_files where database_id = db_id('{db}') and type_desc = 'LOG' '''.format(db=em.database_name))
    size_db = conn.execute('''select (size*8)/1024 as size from sys.master_files where database_id = db_id('{db}') and type_desc = 'ROWS' '''.format(db=em.database_name))
    ...
    logging.info('creating SDE connection file: ' + em.sde)
    if not os.path.exists(em.sde):
        arcpy.CreateArcSDEConnectionFile_management(out_folder_path=os.path.split(em.sde)[0],
                                                    out_name=os.path.split(em.sde)[1],
                                                    server='TDB03',
                                                    service='sde:sqlserver:TDB03',
                                                    database=em.database_name,
                                                    account_authentication="OPERATING_SYSTEM_AUTH",
                                                    version="dbo.DEFAULT",
...
if __name__ == '__main__':
    ...
    with log.LogConsole(level='WARNING'):
        ...
        with log.LogFile(log_file, log_path):
            logging.info('start')
            try:
                create_EMsde_db(estamap_version, auth_file)
